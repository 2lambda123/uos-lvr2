...
void saveMetaInformation(const std::string &outfile, const YAML::Node &node) {
  ...
  if (p.extension() == ".yaml") {
    std::cout << timestamp << "SaveMetaInformation(YAML): " << outfile
              << std::endl;
    ...
    if (node["pose_estimate"]) {
      ...
      std::cout << timestamp << "SaveMetaInformation(SLAM6D): " << poseOutPath
                << std::endl;
    ...
    if (node["registration"]) {
      ...
      boost::filesystem::path outfilePath(outfile);
      boost::filesystem::path dir = outfilePath.parent_path();
      boost::filesystem::path framesPath(outfilePath.stem().string() +
                                         ".frames");
      boost::filesystem::path framesOutPath = p.parent_path() / framesPath;
      std::cout << timestamp << "SaveMetaInformation(SLAM6D): " << framesOutPath
                << std::endl;
...
YAML::Node loadMetaInformation(const std::string &in) {
  ...
  if (inPath.extension() == ".yaml") {
    ...
    if (boost::filesystem::exists(inPath)) {
      std::cout << timestamp << "LoadMetaInformation(YAML): Loading " << inPath
                << std::endl;
      n = YAML::LoadFile(inPath.string());
    } else {
      std::cout << timestamp
                << "LoadMetaInformation(YAML): Unable to find yaml file: "
                << inPath << std::endl;
    }
    return n;
    ...
    if (in_str.good()) {
      std::cout << timestamp << "LoadMetaInformation(SLAM6D): Loading "
                << poseInPath << std::endl;
    ...
    if (boost::filesystem::exists(framesInPath)) {
      std::cout << timestamp << "LoadMetaInformation(SLAM6D): Loading "
                << framesInPath << std::endl;
      Transformd registration =
          getTransformationFromFrames<double>(framesInPath);
