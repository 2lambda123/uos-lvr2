...
void DirectoryKernel::saveMeshBuffer(const std::string &group,
  ...
  model->m_mesh = buffer;
  std::cout << timestamp << "Directory Kernel::saveMeshBuffer(): " << p.string()
            << std::endl;
  ModelFactory::saveModel(model, p.string());
}

void DirectoryKernel::savePointBuffer(const std::string &group,
                                      const std::string &container,
  ...
  std::cout << timestamp
            << "Directory Kernel::savePointBuffer(): " << p.string()
            << std::endl;
  ModelFactory::saveModel(model, p.string());
}

void DirectoryKernel::saveImage(const std::string &group,
                                const std::string &container,
  ...
  std::cout << timestamp << "Directory Kernel::saveImage(): " << p.string()
            << std::endl;

  cv::imwrite(p.string(), image);
}

void DirectoryKernel::saveMetaYAML(const std::string &group,
                                   const std::string &container,
  ...
  std::cout << timestamp << "Directory Kernel::saveMetaYAML(): " << p.string()
            << std::endl;
  saveMetaInformation(p.string(), node);
}

MeshBufferPtr
DirectoryKernel::loadMeshBuffer(const std::string &group,
                                const std::string container) const {
  boost::filesystem::path p = getAbsolutePath(group, container);
  std::cout << timestamp << "Directory Kernel::loadMeshBuffer(): " << p.string()
            << std::endl;
...
PointBufferPtr
DirectoryKernel::loadPointBuffer(const std::string &group,
                                 const std::string &container) const {
  boost::filesystem::path p = getAbsolutePath(group, container);
  std::cout << timestamp
            << "Directory Kernel::loadPointBuffer(): " << p.string()
            << std::endl;
  ModelPtr model = ModelFactory::readModel(p.string());
  if (model) {
...
DirectoryKernel::loadImage(const std::string &group,
                           const std::string &container) const {
  boost::filesystem::path p = getAbsolutePath(group, container);
  boost::optional<cv::Mat> opt;
  std::cout << timestamp << "Directory Kernel::loadImage: " << p.string()
            << std::endl;
...
void DirectoryKernel::loadMetaYAML(const std::string &group,
                                   const std::string &container,
                                   YAML::Node &n) const {
  boost::filesystem::path p = getAbsolutePath(group, container);
  std::cout << timestamp << "Directory Kernel::loadMetaYAML: " << p.string()
            << std::endl;
...
bool DirectoryKernel::exists(const std::string &group,
                             const std::string &container) const {
...
void DirectoryKernel::subGroupNames(const std::string &group,
                                    std::vector<string> &subGroupNames) const {
...
DirectoryKernel::getAbsolutePath(const std::string &group,
...
ucharArr DirectoryKernel::loadUCharArray(const std::string &group,
                                         const std::string &container,
                                         std::vector<size_t> &dims) const {
  return loadArray<unsigned char>(group, container, dims);
}

floatArr DirectoryKernel::loadFloatArray(const std::string &group,
                                         const std::string &container,
                                         std::vector<size_t> &dims) const {
  return loadArray<float>(group, container, dims);
}

doubleArr DirectoryKernel::loadDoubleArray(const std::string &group,
                                           const std::string &container,
                                           std::vector<size_t> &dims) const {
  return loadArray<double>(group, container, dims);
}

void DirectoryKernel::saveFloatArray(
...
void DirectoryKernel::saveDoubleArray(
    const std::string &groupName, const std::string &datasetName,
    const std::vector<size_t> &dimensions,
...
void DirectoryKernel::saveUCharArray(
    const std::string &groupName, const std::string &datasetName,
    const std::vector<size_t> &dimensions,
