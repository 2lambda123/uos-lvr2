 ...
 * @details   Supports geometrys compressed using draco
...
/**
 *
 * @brief adds the given data to a new attribute and attaches the attribute to
 * the pointcloud
 *
 ...
 * @tparam size is the number of values linked to a single point in the point
 * cloud
 *
 * @param array contains the data that is attempted to be stored in the
 * attribute
...
int saveAttributeToDraco(ArrayType array, draco::PointCloud *drcPointcloud,
                         ...
                         draco::DataType dracoDataType, size_t numPoints,
                         bool normalized) {
  if (array == nullptr) {
  ...
  attribute.Init(geometryType, size, dracoDataType, normalized,
...
/**
 *
 * @brief pushes the given textures in a int32_t vector to store it in the draco
 * structure
 *
 ...
 */
void saveTextures(std::vector<int32_t> *textureValue, textureArr textures,
                  size_t numTextures) {
 ...
 * @param numTextures number of textures in the textureArr
 */
void saveTextures(std::vector<int32_t> *textureValue,
...
/**
 * @brief transfers a pointcloud of a modelPtr to a draco EncoderBuffer that can
 ...
 * @return unique pointer to a EncodeBuffer containing the draco encoded
 *pointcloud that can be written into a file or a nullptr in case of an error
 **/
std::unique_ptr<draco::EncoderBuffer>
encodePointCloud(ModelPtr modelPtr, draco::Encoder &encoder) {
  draco::PointCloud pointCloud;
  draco::GeometryMetadata *metadata = new draco::GeometryMetadata();

  // assuming number of colors, normals, intensities and confidences are equal
  // to number of points
  size_t numElem;
  ...
  try {
    ...
    saveAttributeToDraco<floatArr, float, 3>(
        coordinates, &pointCloud, draco::GeometryAttribute::Type::POSITION,
        draco::DT_FLOAT32, numPoints, false);
  } catch (const std::invalid_argument &ia) {
    std::cerr << "No point coordinates could be found in the pointcloud"
              << std::endl;
  ...
  try {
    ...
    if (normals) {
      saveAttributeToDraco<floatArr, float, 3>(
          normals, &pointCloud, draco::GeometryAttribute::Type::NORMAL,
  ...
  try {
    ...
    if (colors && (w == 3)) {
      ...
      saveAttributeToDraco<ucharArr, unsigned char, 3>(
          colors, &pointCloud, draco::GeometryAttribute::Type::COLOR,
  ...
  //     modelPtr->m_pointCloud->getPointIntensityArray(numElem);

  //     int attId = saveAttributeToDraco<floatArr, float, 1>(
  //         confidences, &pointCloud, draco::GeometryAttribute::Type::GENERIC,
  //         draco::DT_FLOAT32, numElem, false);

  //     draco::AttributeMetadata* attributeMeta = new
  //     draco::AttributeMetadata(); attributeMeta->set_att_unique_id(attId);
  ...
  //     int attId = saveAttributeToDraco<floatArr, float, 1>(
  //         intensities, &pointCloud, draco::GeometryAttribute::Type::GENERIC,
  //         draco::DT_FLOAT32, numElem, false);
  ...
  if (!status.ok()) {
    std::cerr << "An error occurred:" << " " << status.error_msg_string()
              << std::endl;
    return std::unique_ptr<draco::EncoderBuffer>(nullptr);
  }
...
/**
 * @brief transfers a mesh of a modelPtr to a draco EncoderBuffer that can be
 *written into a file
 *
 * @param modelPtr pointer to model thats mesh shall be encoded
 * @param encoder is used to encode the modelptr to a encodeBuffer
 * @return unique pointer to a EncodeBuffer containing the draco encoded mesh
 *that can be written into a file or a nullptr in case of an error
 **/
std::unique_ptr<draco::EncoderBuffer> encodeMesh(ModelPtr modelPtr,
  ...
  // floatArr vertexConfidences =
  // modelPtr->m_mesh->getVertexConfidenceArray(numVertexConfidences);

  // size_t   numVertexIntensities;
  // floatArr vertexIntensities =
  // modelPtr->m_mesh->getVertexIntensityArray(numVertexIntensities);
  ...
  // if there are material indices which exist per face, index draco mesh by
  // face if not, index draco mesh by vertex:
  bool faceIndexed = (faceMaterialIndices != 0);

  if (numVertices > 0) {
    draco::GeometryAttribute attribute;
    attribute.Init(draco::GeometryAttribute::POSITION, nullptr, 3,
                   draco::DT_FLOAT32, false, sizeof(float) * 3, 0);
    verticesAttId = mesh.AddAttribute(attribute, true,
                                      faceIndexed ? numFaces * 3 : numVertices);
    mesh.SetAttributeElementType(
        verticesAttId, draco::MeshAttributeElementType::MESH_VERTEX_ATTRIBUTE);
  }

  if (hasVertexNormals) {
    draco::GeometryAttribute attribute;
    attribute.Init(draco::GeometryAttribute::NORMAL, nullptr, 3,
                   draco::DT_FLOAT32, true, sizeof(float) * 3, 0);
    vertexNormalsAttId = mesh.AddAttribute(
        attribute, true, faceIndexed ? numFaces * 3 : numVertices);
    mesh.SetAttributeElementType(
        vertexNormalsAttId,
  ...
  if (hasVertexColors) {
    draco::GeometryAttribute attribute;
    attribute.Init(draco::GeometryAttribute::COLOR, nullptr, 3, draco::DT_UINT8,
                   false, sizeof(uint8_t) * 3, 0);
    vertexColorsAttId = mesh.AddAttribute(
        attribute, true, faceIndexed ? numFaces * 3 : numVertices);
    mesh.SetAttributeElementType(
        vertexColorsAttId,
        draco::MeshAttributeElementType::MESH_VERTEX_ATTRIBUTE);
  ...
  // {
  //     draco::GeometryAttribute attribute;
  //     attribute.Init(draco::GeometryAttribute::GENERIC, nullptr, 1,
  //     draco::DT_FLOAT32, false,
  //                    sizeof(float), 0);
  //     vertexConfidencesAttId =
  //         mesh.AddAttribute(attribute, true, faceIndexed ? numFaces * 3 :
  //         numVertexConfidences);
  ...
  if (vertexTextureCoordinates) {
    draco::GeometryAttribute attribute;
    attribute.Init(draco::GeometryAttribute::TEX_COORD, nullptr, 3,
                   draco::DT_FLOAT32, false, sizeof(float) * 3, 0);
    vertexTextureCoordinatesAttId = mesh.AddAttribute(
        attribute, true,
        faceIndexed ? numFaces * 3 : numVertexTextureCoordinates);
    mesh.SetAttributeElementType(
  ...
  if (faceMaterialIndices) {
    ...
    faceMaterialIndicesAttId = mesh.AddAttribute(
        attribute, true, faceIndexed ? numFaces * 3 : numFaces);
    mesh.SetAttributeElementType(
    ...
    metadata->AddAttributeMetadata(
        std::unique_ptr<draco::AttributeMetadata>(attributeMeta));
  }
  ...
  if (faceIndexed) {
    ...
    for (draco::FaceIndex i(0); i < numFaces; i++) {
      // TODO: check for illegal accesses of original arrays using the array
      // size
      ...
      if (hasVertexColors) {
        for (int j = 0; j < 3; j++) {
          ...
          mesh.attribute(vertexColorsAttId)
              ->SetAttributeValue(draco::AttributeValueIndex(face[j].value()),
                                  color);
        }
      }
    ...
    for (draco::AttributeValueIndex i(0); i < numVertices; i++) {
      mesh.attribute(verticesAttId)
          ->SetAttributeValue(i, vertices.get() + i.value() * 3);
    }
...
std::unique_ptr<draco::EncoderBuffer>
encodeDraco(ModelPtr modelPtr, draco::EncodedGeometryType type) {
  draco::Encoder encoder;
  // configure encoder
  ...
  if (type == draco::TRIANGULAR_MESH) {
    ...
    // configure this only for pointclounds because it causes loss in visuals of
    // faces on a plain surface
