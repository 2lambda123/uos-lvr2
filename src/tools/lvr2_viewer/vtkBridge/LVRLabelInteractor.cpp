...
void LVRLabelInteractorStyle::OnRightButtonUp() { calculateSelection(false); }
void LVRLabelInteractorStyle::OnLeftButtonUp() { calculateSelection(true); }
...
void LVRLabelInteractorStyle::calculateSelection(bool select) {
  ...
  m_selectedIds = vtkIdTypeArray::SafeDownCast(
      selected->GetPointData()->GetArray("OriginalIds"));
...
void LVRLabelInteractorStyle::OnKeyUp() {
  ...
  if (key == "Left") {
    ...
    if (accepted) {
      ...
      for (vtkIdType i = 0; i < m_selectedIds->GetNumberOfTuples(); i++) {
        ...
        if (labelIndex != 0 && m_pointLabels[id] != 0 &&
      ...
      m_SelectedPoints =
          std::vector<bool>(m_points->GetNumberOfPoints(), false);
      auto newActor = vtkSmartPointer<vtkActor>::New();
      newActor = SelectedActor;
      newActor->GetProperty()->SetColor(
          colors[labelIndex - 1][0] / 255.0, colors[labelIndex - 1][1] / 255.0,
          colors[labelIndex - 1][2] / 255.0); //(R,G,B)
  ...
  if (key == "m") {
    bool accepted;
    QString label = QInputDialog::getItem(
        0, "Modify Selected Points",
        "Choose Label which should be modified:", m_labelList, 0, true,
        &accepted);
    if (accepted) {
  ...
  if (key == "n") {
    ...
    QString text = QInputDialog::getText(
        0, "Insert New Label", "New Label:", QLineEdit::Normal, "", &accepted);
    if (accepted) {
